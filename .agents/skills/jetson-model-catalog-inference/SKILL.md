...
1. **File location:** `src/content/models/<slug>.md` (e.g. `qwen3-vl-8b.md` ΓåÆ `/models/qwen3-vl-8b`).
...
- **`run_commands_by_module`**: map from module id ΓåÆ command string for **Run** matrix cells.
...
- [ ] Required model fields present (`title`, `short_description`, `vision_capable`, `memory_requirements`, `precision`, `model_size`, ΓÇª).
